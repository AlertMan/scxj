...
public class TrackTaskDao extends DBHelper {
	public TrackTaskDao(Context context) {
		super(context);
	}
	 ...
	 * @return
	 */
	public List<TB_TRACK> getAllTrackListByUserName(String userName) {
		...
		try {
			...
			for(TB_TASK task : taskList){
				String dbPath = userName + "/t" + task.getTASKID() + "/TRNTASKRET.DB";
	...
	private int getAllCnt(String taskname, String taskId, String userName) {
		...
		try {
			...
			open(dbPath);
			String sql = "select count(*) as cnt from TB_TRACK where TASKNAME='"+taskname+"'";
	...
	private TB_TRACK getLastItem(String taskname,String taskId,String userId) {
		...
		Cursor cursor = null;
		try {
			String dbPath = userId + "/t" + taskId + "/TRNTASKRET.DB";
			open(dbPath);
			String sql = "select * from TB_TRACK where TASKNAME='"+taskname+"' order by TRACKID desc limit 1";
	...
	private TB_TRACK getFirstItem(String taskname,String taskId,String userName) {
		TB_TRACK  item = new  TB_TRACK();
		Cursor cursor = null;
		try {
			String dbPath = userName + "/t" + taskId + "/TRNTASKRET.DB";
			open(dbPath);
			String sql = "select * from TB_TRACK where TASKNAME='"+taskname+"' order by TRACKID asc limit 1";
	...
	public void updateItem(TB_TRACK item,String userId) {
		...
		String dbPath = userId + "/t" + item.getTASKID() + "/TRNTASKRET.DB";
		open(dbPath);
	...
	public void insertItem(TB_TRACK task,String userId) {
		...
		String dbpath =userId+"/t"+task.getTASKID()+"/TRNTASKRET.DB";
		open(dbpath);
	 ...
	 * @return
	 */
	public List<TB_TRACK> getTrackPointsByTaskName(String taskName,String taskId,String userName) {
		List<TB_TRACK> list = new ArrayList<TB_TRACK>();
		Cursor cursor = null;
		try {
			String dbPath = userName + "/t" + taskId + "/TRNTASKRET.DB";
